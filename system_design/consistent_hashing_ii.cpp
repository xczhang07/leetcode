...
public:
    ...
    vector<int> addMachine(int machine_id) {
        ...
        vector<int> ret;
        int count = 0;
        while(count < this->microNumber)
        {
            int id = rand()%this->shardNumber;
            if(storage[id] == -1)
            {
                storage[id] = machine_id;
                ++count;
                ret.push_back(id);
            }
        }
...
private:
    static int shardNumber;
    static int microNumber;
    static vector<int> storage;
};

int Solution::shardNumber;

int Solution::microNumber;

vector<int> Solution::storage;
